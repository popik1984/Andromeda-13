...
/obj/machinery/newscaster/get_ru_names()
	return alist(
		NOMINATIVE = "ньюскастер",
		GENITIVE = "ньюскастера",
		DATIVE = "ньюскастеру",
		ACCUSATIVE = "ньюскастер",
		INSTRUMENTAL = "ньюскастером",
		PREPOSITIONAL = "ньюскастере",
	)
...
/obj/machinery/newscaster/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("setCriminalName")
			var/temp_name = tgui_input_text(user, "Введите имя преступника", "Обработчик оповещений о розыске", "Джон Доу", max_length = MAX_NAME_LEN, multiline = FALSE)
			if(!temp_name)
				return TRUE
		...
		if("setCrimeData")
			var/temp_desc = tgui_input_text(user, "Опишите преступления", "Обработчик оповещений о розыске", "Неизвестно", max_length = MAX_BROADCAST_LEN, multiline = TRUE)
			if(!temp_desc)
				return TRUE
...
/obj/machinery/newscaster/welder_act(mob/living/user, obj/item/tool)
	...
	if(!tool.tool_start_check(user, amount=1))
		return
	user.balloon_alert_to_viewers("начинает заваривать...", "начинает починку...")
	...
	if(!tool.use_tool(src, user, 40, volume=50, extra_checks = CALLBACK(src, PROC_REF(needs_repair))))
		user.balloon_alert_to_viewers("заканчивает заваривать!", "прерывает починку!")
		return
	user.balloon_alert_to_viewers("чинит [src]")
	atom_integrity = max_integrity
	set_machine_stat(machine_stat & ~BROKEN)

/obj/machinery/newscaster/wrench_act(mob/living/user, obj/item/tool)
	to_chat(user, span_notice("Вы начинаете [anchored ? "откручивать" : "прикручивать"] [src]..."))
	if(!tool.use_tool(src, user, 60, volume=50))
		return
	playsound(src, 'sound/items/deconstruct.ogg', 50, TRUE)
	if((machine_stat & BROKEN))
		to_chat(user, span_warning("Сломанные остатки [src] падают на пол."))
		new /obj/item/stack/sheet/iron(loc, 5)
		new /obj/item/shard(loc)
		new /obj/item/shard(loc)
	else
		to_chat(user, span_notice("Вы [anchored ? "открутили" : "прикрутили"] [src]."))
...
/obj/machinery/newscaster/attack_paw(mob/living/user, list/modifiers)
	if(!user.combat_mode)
		to_chat(user, span_warning("Управление ньюскастером слишком сложно для вашего крошечного мозга!"))
	else
...
/obj/machinery/newscaster/proc/attach_photo(mob/user)
	...
	if(issilicon(user))
		...
		if(isAI(user))
			...
			to_chat(user, span_warning("Вы не можете взаимодействовать с загрузкой фото!"))
		if(!targetcam.stored.len)
			to_chat(user, span_bolddanger("Нет сохранённых изображений."))
			return
...
/obj/machinery/newscaster/proc/print_paper(mob/user)
	if(paper_remaining <= 0)
		balloon_alert_to_viewers("закончилась бумага!")
		return TRUE
...
/obj/machinery/newscaster/proc/news_alert(channel, update_alert = TRUE)
	if(channel)
		...
		say("Внимание! Распространяется объявление о розыске!")
...
/obj/machinery/newscaster/proc/create_channel(mob/user, channel_locked, cross_sector)
	...
	if(potential_channel)
		tgui_alert(user, "ОШИБКА: Новостной канал с таким именем уже существует в сети.", list("Окей"))
		return TRUE

	var/list/hard_filter_result = is_ic_filtered(channel_name)
	if(hard_filter_result)
		tgui_alert(user, "Имя вашего канала содержит: (\"[hard_filter_result[CHAT_FILTER_INDEX_WORD]]\"), что не разрешено на этом сервере.")
		return TRUE

	var/choice = tgui_alert(user, "Пожалуйста, подтвердите создание новостного канала","Обработчик сетевых каналов", list("Подтвердить", "Отмена"))
	creating_channel = FALSE
	...
	if(soft_filter_result)
		if(tgui_alert(user,"Имя вашего канала содержит \"[soft_filter_result[CHAT_FILTER_INDEX_WORD]]\". \"[soft_filter_result[CHAT_FILTER_INDEX_REASON]]\", Вы уверены, что хотите использовать его?", "Слово в мягком блоке", list("Да", "Нет")) != "Да")
			return
		message_admins("[ADMIN_LOOKUPFLW(user)] обошёл мягкий фильтр для слова \"[soft_filter_result[CHAT_FILTER_INDEX_WORD]]\". \
			Возможно, он использует запрещённый термин для межстанционного канала новостей. Увеличиваю время задержки для отклонения.\n\n Имя канала: \"[channel_name]\"")
		log_admin_private("[key_name(user)] обошёл мягкий фильтр для слова \"[soft_filter_result[CHAT_FILTER_INDEX_WORD]]\". \
			Возможно, он использует запрещённый термин для межстанционного канала новостей. Увеличиваю время задержки для отклонения.\n\n Имя канала: \"[channel_name]\"")
	...
	update_static_data(user)
	user.log_message("собирается создать кросс-секторный новостной канал со следующим именем: [channel_name]", LOG_GAME)
	to_chat(
		GLOB.admins,
		span_adminnotice( \
			"<b color='orange'>Создание кросс-секторного канала (ИСХОДЯЩЕЕ):</b> [ADMIN_LOOKUPFLW(user)] собирается создать кросс-секторный \
			новостной канал \"[channel_name]\" (автоодобрение через [DisplayTimeText(approval_time)]): \
			<b><a href='byond://?src=[REF(src)];reject_channel_creation=1'>ОТКЛОНИТЬ</a></b>"\
		)
	)
...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if (!usr.client?.holder)
		usr.log_message("попытался отклонить создание кросс-секторного новостного канала, не будучи администратором.", LOG_ADMIN)
		message_admins("[key_name(usr)] попытался отклонить создание кросс-секторного новостного канала, не будучи администратором.")
		return

	if (isnull(channel_approval_timer))
		to_chat(usr, span_warning("Слишком поздно!"))
		return
...
/obj/machinery/newscaster/proc/start_create_channel(mob/user)
	...
	if(!newscaster_username || (newscaster_username in existing_authors))
		creating_channel = FALSE
		tgui_alert(user, "ОШИБКА: Пользователь не найден или уже владеет новостным каналом.", list("Окей"))
		return TRUE
...
/obj/machinery/newscaster/proc/create_story(mob/user, channel_id)
	...
	var/temp_message = tgui_input_text(user, "Напишите историю для ленты", "Обработчик сетевых каналов", feed_channel_message, max_length = MAX_MESSAGE_LEN, multiline = TRUE)
...
/obj/machinery/newscaster/proc/toggle_photo(mob/user)
	if(current_image)
		balloon_alert(user, "текущее фото удалено.")
		current_image = null
		return TRUE
	...
	if(current_image)
		...
		balloon_alert(user, "фото не обнаружено.")
...
/obj/machinery/newscaster/proc/create_bounty()
	...
	for(var/datum/station_request/iterated_station_request as anything in GLOB.request_list)
		if(iterated_station_request.req_number == current_user.account_id)
			say("Аккаунт уже имеет активный заказ.")
			return TRUE
	...
	for(var/obj/iterated_bounty_board as anything in GLOB.allbountyboards)
		iterated_bounty_board.say("Новый заказ добавлен!")
...
/obj/machinery/newscaster/proc/pay_applicant(datum/bank_account/payment_target)
	...
	say("Выплачено [active_request.value] кредитов.")
	GLOB.request_list.Remove(active_request)
	qdel(active_request)

/obj/item/wallframe/newscaster
	name = "newscaster frame"
	desc = "Используется для сборки ньюскастеров, просто прикрепите к стене."
	icon_state = "newscaster_assembly"
	custom_materials = list(/datum/material/iron= SHEET_MATERIAL_AMOUNT * 7)
	result_path = /obj/machinery/newscaster
	pixel_shift = 30

/obj/item/wallframe/newscaster/get_ru_names()
	return alist(
		NOMINATIVE = "каркас ньюскастера",
		GENITIVE = "каркаса ньюскастера",
		DATIVE = "каркасу ньюскастера",
		ACCUSATIVE = "каркас ньюскастера",
		INSTRUMENTAL = "каркасом ньюскастера",
		PREPOSITIONAL = "каркасе ньюскастера",
	)
