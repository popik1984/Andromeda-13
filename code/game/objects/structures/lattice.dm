/obj/structure/lattice
	name = "lattice"
	desc = "Лёгкая опорная решётка. Они удерживают нашу станцию."
	gender = FEMALE
...
/obj/structure/lattice/get_ru_names()
	return alist(
		...
		DATIVE = "решётке",
		ACCUSATIVE = "решётку",
		INSTRUMENTAL = "решёткой",
		PREPOSITIONAL = "решётке",
	)
...
/obj/structure/lattice/proc/deconstruction_hints(mob/user)
	return span_notice("Похоже, прутья можно <b>разрезать</b>. Здесь есть место для новых <i>прутьев</i> или <i>плитки</i>.")
...
/obj/structure/lattice/attackby(obj/item/C, mob/user, list/modifiers, list/attack_modifiers)
	...
	if(C.tool_behaviour == TOOL_WIRECUTTER)
		to_chat(user, span_notice("Разрезание креплений [RU_SRC_PRE]..."))
		deconstruct()
	else
...
/obj/structure/lattice/catwalk
	name = "catwalk"
	desc = "Мостик для удобного маневрирования в скафандре и прокладки кабелей."
	gender = MALE
...
/obj/structure/lattice/catwalk/deconstruction_hints(mob/user)
	return span_notice("Опорные прутья можно <b>разрезать</b>.")
...
/obj/structure/lattice/catwalk/mining
	...
	desc = "Тяжело укреплённый мостик, используемый для наведения мостов во враждебной среде. Не похоже, что его можно сдвинуть с места."
...
/obj/structure/lattice/catwalk/mining/get_ru_names()
	return alist(
		...
		ACCUSATIVE = "укреплённый мостик",
		INSTRUMENTAL = "укреплённым мостиком",
		PREPOSITIONAL = "укреплённом мостике",
	)
...
/obj/structure/lattice/catwalk/lava
	...
	desc = "Специализированный мостик для прокладки путей через лаву. Смотрите под ноги."
	icon = 'icons/obj/smooth_structures/catwalk.dmi'
	icon_state = "catwalk-0"
	base_icon_state = "catwalk"
	number_of_mats = 1
...
/obj/structure/lattice/catwalk/lava/get_ru_names()
	return alist(
		...
		ACCUSATIVE = "термостойкий мостик",
		INSTRUMENTAL = "термостойким мостиком",
		PREPOSITIONAL = "термостойком мостике",
	)

/obj/structure/lattice/catwalk/lava/deconstruction_hints(mob/user)
	return span_notice("Похоже, прутья можно <b>разрезать</b>, но <i>термопокрытие осыпется</i>. Здесь есть место для <i>плитки</i>.")
...
/obj/structure/lattice/catwalk/lava/attackby(obj/item/attacking_item, mob/user, list/modifiers, list/attack_modifiers)
	...
	if(!attacking_tiles.use(1))
		to_chat(user, span_warning("Вам нужна одна напольная плитка, чтобы строить поверх [RU_SRC_GEN]."))
		return
	to_chat(user, span_notice("Вы сооружаете новое покрытие, используя [RU_SRC_ACC] в качестве опоры."))
...
/obj/structure/lattice/catwalk/boulder
	...
	desc = "Валун, плавающий в расплавленной смертоносной лаве. Больше похоже на ПЛОТформу."
	gender = FEMALE
...
/obj/structure/lattice/catwalk/boulder/get_ru_names()
	return alist(
		...
		ACCUSATIVE = "платформу-валун",
		INSTRUMENTAL = "платформой-валуном",
		PREPOSITIONAL = "платформе-валуне",
	)
...
/obj/structure/lattice/catwalk/boulder/attackby(obj/item/attacking_item, mob/user, list/modifiers, list/attack_modifiers)
	if(ismetaltile(attacking_item))
		balloon_alert(user, "слишком нестабильно!")
		return FALSE
	return ..()
...
/obj/structure/lattice/catwalk/boulder/proc/self_destruct()
	visible_message(span_notice("[src] тонет и исчезает!"))
