...
/turf/closed/wall/examine(mob/user)
	...
	if(initial(smoothing_flags) & SMOOTH_DIAGONAL_CORNERS)
		. += span_notice("Его углы можно настроить <b>гаечным ключом</b>.")
	. += deconstruction_hints(user)

/turf/closed/wall/proc/deconstruction_hints(mob/user)
	return span_notice("Внешняя обшивка <b>приварена</b> намертво.")
...
/turf/closed/wall/attack_hulk(mob/living/carbon/user)
	...
	if(prob(hardness))
		...
		user.say(pick(";РАААААААААААРГХ!", ";ХНННННННННГГГГГГХ!", ";ГВАААААААААРРРХХ!", ";ННННННННГГГГГГГГХХ!", ";АААААААААРРГХ!" ), forced = "hulk")
...
/turf/closed/wall/proc/try_clean(obj/item/W, mob/living/user)
	...
	if(W.tool_behaviour == TOOL_WELDER)
		...
		to_chat(user, span_notice("Вы начинаете заваривать вмятины на стене..."))
		if(W.use_tool(src, user, 0, volume=100))
			if(iswallturf(src) && LAZYLEN(dent_decals))
				to_chat(user, span_notice("Вы завариваете некоторые вмятины на стене."))
