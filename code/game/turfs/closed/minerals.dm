...
/turf/closed/mineral/gibtonite/attackby(obj/item/attacking_item, mob/living/user, list/modifiers, list/attack_modifiers, exp_multiplier = 1)
	...
	if(istype(attacking_item, /obj/item/goliath_infuser_hammer) && stage == GIBTONITE_ACTIVE)
		user.visible_message(span_notice("[RU_USER_NOM] погружает [RU_ACC(attacking_item)] в [RU_SRC_ACC]..."), span_notice("Ваш щупальцевый молот инстинктивно погружается и обвивает [RU_SRC_NOM], чтобы остановить его..."))
		defuse(user)
		...
		user.visible_message(span_notice("[RU_USER_NOM] подносит [RU_ACC(attacking_item)] к [RU_SRC_ACC]..."), span_notice("Вы используете [RU_ACC(attacking_item)], чтобы определить, где прервать цепную реакцию и попытаться остановить её..."))
		defuse(user)
	. = ..()
	if(istype(attacking_item, /obj/item/clothing/gloves/gauntlets) && previous_stage == GIBTONITE_UNSTRUCK && stage == GIBTONITE_ACTIVE && istype(user))
		...
		user.visible_message(span_danger("[RU_USER_NOM] ударил гибтонит [RU_INS(attacking_item)], отбрасывая [GEND_HIM_HER(user)] назад!"), span_danger("Вы ударили гибтонит! Ваш [RU_NOM(attacking_item)] отбросил вас назад!"))
