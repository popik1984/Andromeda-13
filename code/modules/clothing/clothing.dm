...
/obj/item/clothing/proc/disable_zone(def_zone, damage_type)
	...
	switch(zones_disabled)
		if(1)
			name = "повреждённ[GEND_YI_AYA_OE_YE(src)] [initial(name)]"
		if(2)
			name = "облезл[GEND_YI_AYA_OE_YE(src)] [initial(name)]"
		if(3 to INFINITY) // лучше заботься о своём барахле, чувак
			name = "оборванн[GEND_YI_AYA_OE_YE(src)] [initial(name)]"
...
/obj/item/clothing/examine_tags(mob/user)
	...
	if((TRAIT_QUICK_CARRY in clothing_traits) || (TRAIT_QUICKER_CARRY in clothing_traits))
		.["тактильный"] = "Уменьшает требуемое время для поднятия существ на [(TRAIT_QUICKER_CARRY in clothing_traits) ? "2 секунды" : "1 секунду"]."
