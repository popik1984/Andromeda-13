...
/mob/living/throw_item(atom/target)
	...
	var/verb_text = pick("бросает", "кидает", "швыряет", "мечет", "запускает")
	if(prob(0.5))
		verb_text = "запуливает"
...
/mob/living/proc/give(mob/living/offered)
	if(has_status_effect(/datum/status_effect/offering))
		to_chat(src, span_warning("Вы уже что-то предлагаете!"))
		return

	if(IS_DEAD_OR_INCAP(src))
		to_chat(src, span_warning("Вы не можете ничего предложить в вашем текущем состоянии!"))
		return
	...
	if(!offered_item || ((offered_item.item_flags & ABSTRACT) && !(offered_item.item_flags & HAND_ITEM)))
		to_chat(src, span_warning("У вас нет ничего в руках, чтобы предложить!"))
		return
