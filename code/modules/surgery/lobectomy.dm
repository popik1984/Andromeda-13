...
/datum/surgery_step/lobectomy/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	if(ishuman(target))
		...
		display_results(
			user,
			target,
			span_notice("Вы успешно вырезаете самую поврежденную долю у [human_target]."),
			span_notice("[user] успешно вырезает часть легких у [human_target]."),
			"",
		)
...
/datum/surgery_step/lobectomy/failure(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	if(ishuman(target))
		...
		display_results(
			user,
			target,
			span_warning("Вы ошибаетесь, не сумев вырезать поврежденную долю у [human_target]!"),
