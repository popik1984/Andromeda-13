...
/datum/surgery_step/wing_reconstruction/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(
		user,
		target,
		span_notice("Вы начинаете восстанавливать обугленные мембраны крыльев [target]..."),
		span_notice("[user] начинает чинить обугленные мембраны крыльев [target]."),
		span_notice("[user] начинает проводить операцию на обугленных мембранах крыльев [target]."),
	)
...
/datum/surgery_step/wing_reconstruction/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	if(ishuman(target))
		...
		display_results(
			user,
			target,
			span_notice("Вам удалось восстановить крылья [target]."),
			span_notice("[user] успешно восстанавливает крылья [target]!"),
			span_notice("[user] завершает операцию на крыльях [target]."),
		)
