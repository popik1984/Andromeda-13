...
/datum/surgery_step/bionecrosis/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(
		user,
		target,
		span_notice("Вы начинаете выращивать опухоль ромерола в мозге у [target]..."),
		span_notice("[user] начинает возиться с мозгом у [target]..."),
...
/datum/surgery_step/bionecrosis/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(
		user,
		target,
		span_notice("Вам удалось вырастить опухоль ромерола в мозге у [target]."),
		span_notice("[user] успешно выращивает опухоль ромерола в мозге у [target]!"),
