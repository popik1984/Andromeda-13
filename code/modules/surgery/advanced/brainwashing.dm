...
/obj/item/disk/surgery/brainwashing/get_ru_names()
	return list(
		...
		ACCUSATIVE = "диск промывки мозгов",
		INSTRUMENTAL = "диском промывки мозгов",
		PREPOSITIONAL = "диске промывки мозгов",
	)
...
/datum/surgery_step/brainwash/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	if(!target.mind)
		to_chat(user, span_warning("[target] не реагирует на промывание мозгов, как будто [GEND_HE_SHE(target)] лишен[GEND_A_O_I(target)] разума..."))
		return FALSE
	if(HAS_MIND_TRAIT(target, TRAIT_UNCONVERTABLE))
		to_chat(user, span_warning("Вы слышите слабое жужжание от устройства внутри мозга [target], и результаты промывание мозгов стираются."))
		return FALSE
	display_results(
		user,
		target,
		span_notice("Вам удалось промыть мозги [target]."),
		span_notice("[user] успешно восстанавливает мозг у [target]!"),
