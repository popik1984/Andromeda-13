...
/datum/surgery_step/apply_bioware/ground_nerves/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(
		user,
		target,
		span_notice("Вы начинаете перенаправлять нервы [target]."),
		span_notice("[user] начинает перенаправлять нервы [target]."),
		span_notice("[user] начинает манипулировать нервной системой [target]."),
	)
...
/datum/surgery_step/apply_bioware/ground_nerves/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(
		user,
		target,
		span_notice("Вы успешно перенаправляете нервную систему [target]!"),
		span_notice("[user] успешно перенаправляет нервную систему [target]!"),
		span_notice("[user] завершает манипулирование нервной системой [target]."),
	)
