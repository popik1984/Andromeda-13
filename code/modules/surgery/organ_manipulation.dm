...
/datum/surgery_step/manipulate_organs/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(istype(tool, /obj/item/borg/apparatus/organ_storage))
		...
		if(!isorgan(target_organ))
			...
			to_chat(user, span_warning("Вы не можете вставить [target_organ.declent_ru(ACCUSATIVE)] в [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target]!"))
			return SURGERY_STEP_FAIL
		tool = target_organ
	if(isorgan(tool))
		current_type = "insert"
		...
		if(target_zone != target_organ.zone || target.get_organ_slot(target_organ.slot))
			to_chat(user, span_warning("Здесь нет места для [target_organ.declent_ru(GENITIVE)] в [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target]!"))
			return SURGERY_STEP_FAIL
		...
		display_results(
			user,
			target,
			span_notice("Вы вставляете [tool.declent_ru(ACCUSATIVE)] в [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target]..."),
			span_notice("[user] вставляет [tool.declent_ru(ACCUSATIVE)] в [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target]."),
			span_notice("[user] вставляет что-то в [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target]."),
		)
		display_pain(target, "Вы чувствуете, как что-то вставили в вашу [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)]!")
		...
		if (target_zone == BODY_ZONE_PRECISE_EYES)
			target_zone = check_zone(target_zone)
		if(!length(organs))
			to_chat(user, span_warning("В [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] нет органов, которые можно удалить у [target]!"))
			return SURGERY_STEP_FAIL
		else
			...
			if(user && target && user.Adjacent(target))
				...
				display_results(
					user,
					target,
					span_notice("Вы начинаете извлекать [target_organ.declent_ru(ACCUSATIVE)] из [target.parse_zone_with_bodypart(target_zone, GENITIVE)] у [target]..."),
					span_notice("[user] начинает извлекать [target_organ.declent_ru(ACCUSATIVE)] из [target.parse_zone_with_bodypart(target_zone, GENITIVE)] у [target]."),
					span_notice("[user] начинает извлекать что-то из [target.parse_zone_with_bodypart(target_zone, GENITIVE)] у [target]."),
				)
				display_pain(target, "Вы чувствуете, как [target_organ.declent_ru(ACCUSATIVE)] извлекли из вашей  [target.parse_zone_with_bodypart(target_zone, GENITIVE)]!")
			else
				return SURGERY_STEP_FAIL
...
/datum/surgery_step/manipulate_organs/success(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	...
	if(current_type == "insert")
		...
		display_results(
			user,
			target,
			span_notice("Вы устанавливаете [tool.declent_ru(ACCUSATIVE)] в [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target]."),
			span_notice("[user] устанавливает [tool.declent_ru(ACCUSATIVE)] в [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target]!"),
			span_notice("[user] устанавливает что-то в [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target]!"),
		)
		display_pain(target, "Ваша [target.parse_zone_with_bodypart(target_zone, NOMINATIVE)]  болит, пока [tool.declent_ru(NOMINATIVE)] приживается к телу!")
		...
		if(target_organ && target_organ.owner == target)
			display_results(
				user,
				target,
				span_notice("Вы успешно извлекате [target_organ.declent_ru(ACCUSATIVE)] из [target.parse_zone_with_bodypart(target_zone, GENITIVE)] у [target]."),
				span_notice("[user] успешно извлек [target_organ.declent_ru(ACCUSATIVE)] из [target.parse_zone_with_bodypart(target_zone, GENITIVE)] у [target]!"),
				span_notice("[user] успешно извлек что-то из [target.parse_zone_with_bodypart(target_zone, GENITIVE)] у [target]!"),
			)
			display_pain(target, "Ваша [target.parse_zone_with_bodypart(target_zone, NOMINATIVE)] болит, вы больше не чувствуете [target_organ.declent_ru(ACCUSATIVE)]!")
			...
			display_results(
				user,
				target,
				span_warning("Вы не можете ничего извлечь из [target.parse_zone_with_bodypart(target_zone, GENITIVE)] у [target]!"),
				span_notice("[user] похоже не может ничего извлечь из [target.parse_zone_with_bodypart(target_zone, GENITIVE)] у [target]!"),
				span_notice("[user] похоже не может ничего извлечь из [target.parse_zone_with_bodypart(target_zone, GENITIVE)] у [target]!"),
			)
