...
/obj/item/blood_filter
	name = "blood filter"
	desc = "Медицинское устройство для гемофильтрации жидкостей живых организмов."
	icon = 'icons/obj/medical/surgery_tools.dmi'
...
/obj/item/blood_filter/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("add")
			var/selected_reagent = tgui_input_list(usr, "Выбрать реагент для фильтрации", "Внести реагент в белый список", GLOB.name2reagent)
			if(!selected_reagent)
				return FALSE
...
/**
 * TODO: Rewokin: Не очень понял, что это, я хумен мейнер. Если это не относится к хирургическому инструменту, как инструмент выше,
 * то сообщите :)
 */

/obj/item/surgical_processor //позволяет медицинским киборгам сканировать и инициировать сложные операции
...
/**
 * MARK: ПОД ВОПРОСОМ!
 * TODO: Rewokin: просто копия стандартных инструментов, но с флагом CRUEL_IMPLEMENT,
 * скорее всего будет убрано, а флаг перемещён на основные инструменты.
 */

/*
 * Жестокие хирургические инструменты
 * Этот вид инструментов имеет флаг CRUEL_IMPLEMENT.
 * Бонусы, если операция выполняется мрачным пользователем и это соответствует его интересам.
 * Мрачных пользователей интересуют: аутопсии, операции по оживлению, пластическая хирургия, манипуляции с органами/особенностями, ампутации
 * В остальном - обычный инструмент.
 */
