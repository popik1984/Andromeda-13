...
/datum/surgery_step/revive/preop(mob/user, mob/living/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(
		user,
		target,
		span_notice("Вы готовитесь применить [tool.declent_ru(ACCUSATIVE)] для воздействия на мозг [target]."),
		span_notice("[user] готовится применить [tool.declent_ru(ACCUSATIVE)] для воздействия на мозг [target]."),
		span_notice("[user] готовится применить [tool.declent_ru(ACCUSATIVE)] для воздействия на мозг [target]."),
	)
...
/datum/surgery_step/revive/success(mob/user, mob/living/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results)
	display_results(
		user,
		target,
		span_notice("Вы успешно поражаете мозг [target] с помощью [tool.declent_ru(GENITIVE)]..."),
		span_notice("[user] посылает мощный удар в мозг [target] с помощью  [tool.declent_ru(GENITIVE)]..."),
		span_notice("[user] посылает мощный удар в мозг [target] с помощью  [tool.declent_ru(GENITIVE)]..."),
	)
	target.grab_ghost()
	...
	target.visible_message(span_warning("...[target] бьется в конвульсиях, после чего замирает.."))
	return FALSE
...
/datum/surgery_step/revive/failure(mob/user, mob/living/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(
		user,
		target,
		span_notice("Вы успешно поражаете мозг [target] при помощи [tool.declent_ru(GENITIVE)], но [GEND_HE_SHE(target)] не реагирует."),
		span_notice("[user] успешно поражает мозг [target] мощным ударом с помощью [tool.declent_ru(GENITIVE)], но [GEND_HE_SHE(target)] не реагирует."),
		span_notice("[user] успешно поражает мозг [target] мощным ударом с помощью [tool.declent_ru(GENITIVE)], но [GEND_HE_SHE(target)] не реагирует."),
	)
	return FALSE
