...
/obj/item/bodypart/proc/check_for_injuries(mob/living/carbon/human/examiner)
	...
	if(current_gauze)
		...
		switch(cached_bleed_rate)
			if(0.2 to 1)
				bleed_text += span_warning("\tОна слегка кровоточит.")
			if(1 to 2)
				bleed_text += span_warning("\tОна кровоточит.")
			if(3 to 4)
				bleed_text += span_warning("\tОна сильно кровоточит!")
			if(4 to INFINITY)
				bleed_text += span_warning("\tОна истекает кровью!")
