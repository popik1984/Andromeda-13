...
/datum/surgery_step/add_prosthetic/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(isbodypart(tool))
		...
		if(!bodypart_to_attach.can_attach_limb(target))
			target.balloon_alert(user, "это не подходит к [target.parse_zone_with_bodypart(target_zone, DATIVE)]!")
			return SURGERY_STEP_FAIL
		...
		if(target_zone == bodypart_to_attach.body_zone) //so we can't replace a leg with an arm, or a human arm with a monkey arm.
			display_results(
				user,
				target,
				span_notice("Вы начинаете заменять [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target] на [tool.declent_ru(ACCUSATIVE)]..."),
				span_notice("[user] начинает заменять [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target] на [tool.declent_ru(ACCUSATIVE)]."),
				span_notice("[user] начинает заменять [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target]."),
			)
		else
			to_chat(user, span_warning("[capitalize(tool.declent_ru(NOMINATIVE))] не подходит для [target.parse_zone_with_bodypart(target_zone, GENITIVE)]."))
			return SURGERY_STEP_FAIL
	else if(target_zone == BODY_ZONE_L_ARM || target_zone == BODY_ZONE_R_ARM)
		display_results(
			user,
			target,
			span_notice("Вы начинаете прикреплять [tool.declent_ru(ACCUSATIVE)] к [target]..."),
			span_notice("[user] начинает прикреплять [tool.declent_ru(ACCUSATIVE)] к [target.parse_zone_with_bodypart(target_zone, DATIVE)] у [target]."),
			span_notice("[user] начинает прикреплять что-то к  [target.parse_zone_with_bodypart(target_zone, DATIVE)] у [target]."),
		)
	else
...
/datum/surgery_step/add_prosthetic/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	if(istype(tool, /obj/item/borg/apparatus/organ_storage))
		...
		to_chat(user, span_warning("Кажется, ты не можешь расстаться с [tool.declent_ru(INSTRUMENTAL)]!"))
		return FALSE
...
/datum/surgery_step/add_prosthetic/proc/handle_bodypart(mob/user, mob/living/carbon/target, obj/item/bodypart/bodypart_to_attach, target_zone)
	...
	display_results(
		...
		span_notice("[user] успешно заменяет [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target] на [bodypart_to_attach.declent_ru(ACCUSATIVE)]!"),
		span_notice("[user] успешно заменяет [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)] у [target]!"),
	)
	display_pain(target, "Вы наполняетесь позитивными ощущениями, потому что вы снова чувствуете вашу [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)], which you can feel again!", TRUE)
...
/datum/surgery_step/add_prosthetic/proc/handle_arbitrary_prosthetic(mob/user, mob/living/carbon/target, obj/item/thing_to_attach, target_zone)
	...
	display_results(
		...
		span_notice("[user] прикрепляет [thing_to_attach.declent_ru(ACCUSATIVE)]!"),
		span_notice("[user] завершает операцию по прикреплению!"),
	)
	display_pain(target, "Вы испытываете странные ощущения от присоединения вашей [target.parse_zone_with_bodypart(target_zone, GENITIVE)]!", TRUE)

/datum/surgery_step/secure_arbitrary_prosthetic
	name = "надежный протез (шов/лента)"
	implements = list(
...
/datum/surgery_step/secure_arbitrary_prosthetic/preop(mob/user, mob/living/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	display_results(
		user, target,
		span_notice("Вы начинаете прикреплять [limb.declent_ru(ACCUSATIVE)] к телу [target]."),
		span_notice("[user] начинает прикреплять [limb.declent_ru(ACCUSATIVE)] к телу [target]."),
		span_notice("[user] начинает что-то делать с телом [target]."),
	)
	display_pain(target, "[user] начинает присоединять [limb.declent_ru(ACCUSATIVE)] к вашему телу!", TRUE)
...
/datum/surgery_step/secure_arbitrary_prosthetic/success(mob/user, mob/living/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	...
	display_results(
		user, target,
		span_notice("Вы прикрепили [limb.declent_ru(ACCUSATIVE)] к телу [target]."),
		span_notice("[user] прикрепил[GEND_A_O_I(target)] [limb.declent_ru(ACCUSATIVE)] к телу [target]!"),
		span_notice("[user] сделал что-то с телом [target]!"),
	)
	display_pain(target, "[user] присоединил[GEND_A_O_I(target)] [limb.declent_ru(ACCUSATIVE)] к вашему телу!", TRUE)
	return TRUE
