...
/datum/surgery_step/repair_bone_hairline/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	if(surgery.operated_wound)
		display_results(
			user,
			target,
			span_notice("Вы приступаете к устранению перелома в [target.parse_zone_with_bodypart(user.zone_selected, PREPOSITIONAL)] у [target]..."),
			span_notice("[user] приступает к устранению перелома в [target.parse_zone_with_bodypart(user.zone_selected, PREPOSITIONAL)] у [target] при помощи [tool.declent_ru(GENITIVE)]."),
			span_notice("[user] приступает к устранению перелома в [target.parse_zone_with_bodypart(user.zone_selected, PREPOSITIONAL)] у [target]."),
		)
...
/datum/surgery_step/repair_bone_hairline/success(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	if(surgery.operated_wound)
		...
		display_results(
			user,
			target,
			span_notice("Вы успешно устраняете перелом в [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target]."),
			span_notice("[user] успешно устраняет перелом в [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target] при помощи [tool.declent_ru(GENITIVE)]!"),
			span_notice("[user] успешно устраняет перелом в [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target]!"),
		)
		log_combat(user, target, "repaired a hairline fracture in", addition="COMBAT_MODE: [uppertext(user.combat_mode)]")
		qdel(surgery.operated_wound)
	else
		to_chat(user, span_warning("У [target] нет перелома в [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)]!"))
	return ..()
...
/datum/surgery_step/reset_compound_fracture/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	if(surgery.operated_wound)
		display_results(
			user,
			target,
			span_notice("Вы начинаете восстанавливать кость в [target.parse_zone_with_bodypart(user.zone_selected, PREPOSITIONAL)] у [target]..."),
			span_notice("[user] начинает восстанавливать кость в [target.parse_zone_with_bodypart(user.zone_selected, PREPOSITIONAL)] у [target] при помощи [tool.declent_ru(GENITIVE)]."),
			span_notice("[user] начинает восстанавливать кость в [target.parse_zone_with_bodypart(user.zone_selected, PREPOSITIONAL)] у [target]."),
		)
		display_pain(target, "Острая боль в [target.parse_zone_with_bodypart(user.zone_selected, PREPOSITIONAL)] просто невыносима!")
	else
		user.visible_message(span_notice("[user] ищет у [target] в [target.parse_zone_with_bodypart(user.zone_selected, PREPOSITIONAL)]."), span_notice("Вы ищете у [target] в [target.parse_zone_with_bodypart(user.zone_selected, PREPOSITIONAL)]..."))
...
/datum/surgery_step/reset_compound_fracture/success(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	if(surgery.operated_wound)
		...
		display_results(
			user,
			target,
			span_notice("Вы успешно восстанавливаете кость в [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target]."),
			span_notice("[user] успешно восстанавливает кость в [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target] при помощи [tool.declent_ru(GENITIVE)]!"),
			span_notice("[user] успешно восстанавливает кость в [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target]!"),
		)
		log_combat(user, target, "reset a compound fracture in", addition="COMBAT MODE: [uppertext(user.combat_mode)]")
	else
		to_chat(user, span_warning("У [target] нет открытого перелома в [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)]!"))
	return ..()
...
/datum/surgery_step/clamp/vessels/discard_skull_debris/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(
		user,
		target,
		span_notice("Вы начинаете избавляться от мелких обломков черепа в [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target]..."),
		span_notice("[user] начинает избавляться от мелких обломков черепа в [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target]..."),
		span_notice("[user] начинает копошиться в [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target]..."),
	)
...
/datum/surgery_step/repair_skull/preop(mob/user, mob/living/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	display_results(
		...
		span_notice("[user] приступает к восстановлению черепа у [target] при помощи [tool.declent_ru(GENITIVE)]."),
...
/datum/surgery_step/repair_skull/success(mob/user, mob/living/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results)
	...
	display_results(
		...
		span_notice("[user] успешно восстанавливает череп у [target] при помощи [tool.declent_ru(GENITIVE)]."),
