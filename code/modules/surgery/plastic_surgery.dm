...
/obj/item/disk/surgery/advanced_plastic_surgery/get_ru_names()
	return list(
		...
		ACCUSATIVE = "диск продвинутой пластической хирургии",
		INSTRUMENTAL = "диском продвинутой пластической хирургии",
		PREPOSITIONAL = "диске продвинутой пластической хирургии",
	)
...
/datum/surgery_step/insert_plastic/preop(mob/user, mob/living/target, target_zone, obj/item/stack/tool, datum/surgery/surgery)
	display_results(
		user,
		target,
		span_notice("Вы начинаете вставлять [tool.declent_ru(ACCUSATIVE)] в разрезе на [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target]..."),
		span_notice("[user] начинает вставлять [tool.declent_ru(ACCUSATIVE)] в разрезе на [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target]."),
		span_notice("[user] начинает вставлять [tool.declent_ru(ACCUSATIVE)] в разрезе на[target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target]."),
	)
	display_pain(target, "Вы чувствуете, как что-то вставили вам под кожу в [target.parse_zone_with_bodypart(target_zone, ACCUSATIVE)].")
...
/datum/surgery_step/reshape_face/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	if(HAS_TRAIT_FROM(target, TRAIT_DISFIGURED, TRAIT_GENERIC))
		...
		display_results(
			user,
			target,
			span_notice("Вы полностью изменяете внешность у [oldname], [GEND_HE_SHE(target)] теперь [newname]."),
			span_notice("[user] полностью изменяет внешность у [oldname], [GEND_HE_SHE(target)] теперь [newname]!"),
			span_notice("[user] заканчивает проводить операцию на лице у [target]."),
		)
