...
/datum/surgery_step/cut_fat/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	display_results(
		...
		span_notice("[user] начинает отрезать что-то из [target.parse_zone_with_bodypart(target_zone, GENITIVE)] у [target] при помощи [tool]."),
	)
	display_pain(target, "Вы чувствуете резкую боль в вашей [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)]!")
...
/datum/surgery_step/cut_fat/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results)
	display_results(
		...
		span_notice("[user] заканчивает разрез на [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] у [target]."),
	)
	display_pain(target, "Жир в вашей [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)] ослабевает, свисает и болит, как черт знает что!")
	return TRUE
...
/datum/surgery_step/remove_fat/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(
		...
		span_notice("[user]начинает извлекать что-то у [target] из [target.parse_zone_with_bodypart(target_zone, PREPOSITIONAL)]."),
	)
