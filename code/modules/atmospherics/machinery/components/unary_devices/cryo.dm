...
/obj/machinery/cryo_cell/get_ru_names()
	return list(
		...
		DATIVE = "криокапсуле",
		ACCUSATIVE = "криокапсулу",
		INSTRUMENTAL = "криокапсулой",
		PREPOSITIONAL = "криокапсуле",
	)
...
/obj/machinery/cryo_cell/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	...
	switch(held_item.tool_behaviour)
		...
		if(TOOL_WRENCH)
			context[SCREENTIP_CONTEXT_LMB] = "[panel_open ? "Вращать" : ""]"
	return CONTEXTUAL_SCREENTIP_SET
...
/obj/machinery/cryo_cell/examine(mob/user) //this is leaving out everything but efficiency since they follow the same idea of "better beaker, better results"
	...
	if(in_range(user, src) || isobserver(user))
		...
		if(beaker)
			...
			. += span_warning("В ней нет мензурки.")
...
/obj/machinery/cryo_cell/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	if(!istype(tool, /obj/item/reagent_containers/cup))
		return
	if(!QDELETED(beaker))
		balloon_alert(user, "стакан внутри!")
...
/obj/machinery/cryo_cell/screwdriver_act(mob/living/user, obj/item/tool)
	...
	if(on)
		...
		return
	if(occupant)
		balloon_alert(user, "пациент внутри!")
		return
...
/obj/machinery/cryo_cell/wrench_act(mob/living/user, obj/item/tool)
	...
	if(occupant)
		...
		return
	if(state_open)
		balloon_alert(user, "закройте сначала!")
		return
...
/obj/machinery/cryo_cell/container_resist_act(mob/living/user)
	...
	if(do_after(user, CRYO_BREAKOUT_TIME, target = src, hidden = TRUE))
		...
		user.visible_message(span_warning("[user] успешно вырывается из [src]!"), \
			span_notice("Вы успешно вырвались из [src]!"))
		open_machine()
...
/obj/machinery/cryo_cell/mouse_drop_receive(mob/target, mob/user, params)
	...
	user.visible_message(span_notice("[user] начинает запихивать [target] в [src]."), span_notice("Вы начинаете запихивать [target] в [src]."))
...
/datum/aas_config_entry/medical_cryo_announcements
	name = "Медицинская тревога: Отчёты криогеники"
	announcement_lines_map = list(
		"Autoejecting" = "Автоизвлечение пациента.",
		"Deceased" = "Отчёт криогеники: Пациент скончался. %AUTOEJECTING",
		"Fully Recovered" = "Отчёт криогеники: Пациент полностью восстановлен. %AUTOEJECTING",
		"Insufficient Gas" = "Отчёт криогеники: Недостаточно криогенного газа, отключение. %AUTOEJECTING",
		"Wound Treatment" = "Отчёт криогеники: Показатели пациента полностью восстановлены, продолжается автоматическое лечение ран."
	)
	vars_and_tooltips_map = list(
		"AUTOEJECTING" = "будет заменено строкой автоизвлечения, если система сообщит о его необходимости"
	)
