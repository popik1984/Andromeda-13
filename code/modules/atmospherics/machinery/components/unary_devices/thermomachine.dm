...
/obj/machinery/atmospherics/components/unary/thermomachine/examine(mob/user)
	...
	. += span_notice(" -Нажмите ЛКМ гаечным ключом, чтобы повернуть [RU_SRC_ACC], или ПКМ, чтобы [anchored ? "открутить" : "прикрутить"].")
...
/obj/machinery/atmospherics/components/unary/thermomachine/multitool_act_secondary(mob/living/user, obj/item/tool)
	...
	visible_message(span_notice("[user] меняет цвет труб [RU_SRC_GEN] на [GLOB.pipe_color_name[pipe_color]]."), ignored_mobs = user)
	to_chat(user, span_notice("Вы меняете цвет труб [RU_SRC_GEN] на [GLOB.pipe_color_name[pipe_color]]."))
	if(anchored)
		reconnect_nodes()
	update_appearance(UPDATE_ICON)
	return ITEM_INTERACT_SUCCESS
