/obj/machinery/airalarm/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	...
	if(held_item.tool_behaviour == TOOL_CROWBAR)
		if(buildstage == AIR_ALARM_BUILD_NO_WIRES)
			context[SCREENTIP_CONTEXT_LMB] = "Выковырять электронику"
		...
		if (panel_open)
			context[SCREENTIP_CONTEXT_LMB] = "Манипулировать проводами"

	else if(held_item.tool_behaviour == TOOL_WRENCH)
		if(buildstage == AIR_ALARM_BUILD_NO_CIRCUIT)
			context[SCREENTIP_CONTEXT_LMB] = "Открепить тревогу"
